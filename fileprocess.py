...
def generate_analysis_report(project_path=".", output_file="report.md"):
    ...
    try:
        ...
        # 使用CLI相同的方法生成详细报告
        print("[生成] 生成详细Markdown报告...")
        reporter.export_to_markdown(output_file)

        print(f"[完成] 报告已保存到: {output_file}")

        # 获取文件大小信息
        try:
            file_size = os.path.getsize(output_file)
            print(f"[信息] 报告文件大小: {file_size} 字节")
        except:
            pass
...
def load_report():
    ...
    try:
        ...
        print(f"[OK] 已加载report.md (大小: {len(report_content)} 字符)")
        return report_content
    except FileNotFoundError:
        print("[ERROR] 未找到report.md文件")
        return None
    except Exception as e:
        print(f"[ERROR] 加载report.md失败: {e}")
        return None
...
"""
    ...
    print(f"[AI响应] 收到响应，长度: {len(ai_response)} 字符")
    print(f"[AI响应内容]\n{ai_response}")
    ...
    if libraries_info:
        print(
            f"[成功] 解析到 {len(libraries_info.get('third_party_libraries', []))} 个库的信息"
        )
        return {
        ...
        for filename, data in files_data.items():
            ...
            try:
                with open(filepath, "w", encoding="utf-8") as f:
                    f.write(data)
                print(f"[OK] 已保存: {filepath} (大小: {len(data)} 字符)")
            except Exception as e:
                print(f"[ERROR] 保存失败: {filepath} - 错误: {e}")

        return summary, tree, content
    except Exception as e:
        print(f"[ERROR] 获取仓库 {repo_name} 失败: {e}")
...
async def get_all_repos_context(github_links):
    ...
    print(f"开始异步处理 {len(github_links)} 个仓库...")
...
# AI配置信息
AI_CONFIG = {
    "api_key": "sk-ai-v1-9bc874ae703409ecb8562495c851743aaf3656e5bda396078f48c4d0c0aa17be",
...
async def run_complete_analysis(project_path=".", output_file="report.md"):
    ...
    if result:
        print("\n" + "=" * 50)
        print(f"报告生成完成！可以查看 {output_file} 文件")
        print("=" * 50)
